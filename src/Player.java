	...
	public void drawField(Graphics g, int width) {
		...
		for (int i = 0; i < field.size(); i++) {
			...
			g.drawString(field.get(i).getAttack() + "/" + field.get(i).getDefense(),
					width / 2 - (width / 20) * (field.size()) + width / 10 * i + 60, 740);
		}
		if (selected < -1) {
			g.setColor(new Color(50, 50, 50, 150));
	...
	public void select(int x, int y, int width, ArrayList<Minion> eField) {
		...
		if (y < 800 && selected > -1) {
			...
			if (c.getMyCost() <= currentMana) {
				...
				default:
					for (int i = 0; i < eField.size(); i++) {
						...
						if (x > width / 2 - (width / 20) * (eField.size()) + width / 10 * (i) && y > 125
								&& x < width / 2 - (width / 20) * (eField.size()) + width / 10 * (i) + width / 10
								&& y < 125 + (int) (width / 10 / Card.aspectRatio)) {
							if (c.getType() == 2)
								c.activate(eField.get(i), this);
							else
			...
			for (int i = 0; i < eField.size(); i++) {
				if (x > width / 2 - (width / 20) * (eField.size()) + width / 10 * (i) && y > 125
						&& x < width / 2 - (width / 20) * (eField.size()) + width / 10 * (i) + width / 10
						&& y < 125 + (int) (width / 10 / Card.aspectRatio) && field.get(-2 - selected).getStun()) {
					field.get(-2 - selected).fight(eField.get(i));
					field.get(-2 - selected).setStun(false);
					if (eField.get(i).getDefense() < 1)
						eField.remove(i);
					if (field.get(-2 - selected).getDefense() <= 0)
						destroyCard(selected);
				}
			}
		}
	}
